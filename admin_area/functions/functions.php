...
///bat dau getcatpro///
    ...
    function getsearch() {
        ...
        $get_products = "SELECT lv.level_name,emp.*,sc.time_in,sc.time_out,ps.position_name
        FROM employee as emp, levels as lv, schedule as sc, position as ps
        WHERE lv.id = emp.id_level 
        AND sc.id = emp.id_schedule
        AND ps.id = emp.id_position	AND ( emp.full_name like '$find' or emp.email like'$find' or emp.employee_id like'$find' or emp.phone like'$find') ";
        ...
        $count = mysqli_num_rows($run_products);
            ...
            while($row_c = mysqli_fetch_array($run_products)) {
                ...
                echo "
                        ...
                        <td> $p_name </td>
                        <td>  $lv_name </td>
                        <td>   date('h:i A',$t_in)  .' - '.  date('h:i A',$t_out) </td>
                        <td>  $c_email  </td>
                        <td> 
